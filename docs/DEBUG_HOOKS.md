...
| `__capturePose()` / **P** | The live camera as replayable `__captureView` values (`view`), plus player, ground clearance, viewport, backend, weather, grass state, biome filter and every material setting. **P** calls `__copyPose()`, which puts that JSON on the clipboard and in the console; `__lastPose` holds it when an unfocused window's clipboard write is refused. The `view` target is 60 m along the look ray, so replaying it reproduces the frame exactly — a capture camera is position + orientation, and the target distance is not part of the pose. |
